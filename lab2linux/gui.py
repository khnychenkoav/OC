...
class BitonicSortGUI(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Label(frame, text="Количество потоков:").grid(
        ...
        run_button = tk.Button(
            frame, text="Запустить сортировку", command=self.run_sort)
        run_button.grid(row=0, column=2, padx=5, pady=5)

        autotest_button = tk.Button(
            frame, text="Автотестирование", command=self.run_autotest)
    ...
    def run_sort(self):
        try:
            ...
            messagebox.showerror(
                "Ошибка", "Введите корректное количество потоков")
            return

        self.output_text.delete('1.0', tk.END)
        self.output_text.insert(tk.END, f"Запуск сортировки с {
                                num_threads} поток(ами)...\n")

        threading.Thread(target=self.execute_sort, args=(num_threads,)).start()

    def execute_sort(self, num_threads):
        command = ["./bitonic_sort", str(num_threads)]
    ...
    def update_output(self, output, num_threads):
        ...
        if match:
            ...
            self.output_text.insert(
                tk.END, "Не удалось получить время сортировки\n")
    ...
    def update_output_autotest(self, output, num_threads):
        ...
        if match:
            ...
            self.output_text.insert(tk.END, f"Не удалось получить время сортировки для {
                                    num_threads} поток(ов)\n")
